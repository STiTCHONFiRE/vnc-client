...
export class VncClientComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.route.data.subscribe(({ tokenData }) => {
  ...
  disconnectEventFn = (event: CustomEvent<{ clean: boolean }>) => {
    if (event.detail.clean) {
      if (isDevMode()) console.log('Disconnected from the server');
    } else {
      this.loading$.next(false);
      this.err$.next(true);
      this.modalClipboard.nativeElement.close();
      this.infoPanel.sendMessage(
        'Ошибка подключения: что-то пошло не так, попробуйте позже',
        'ERR'
      );
    }
  };
  ...
  connectedEventFn = () => {
    this.loading$.next(false);
    this.infoPanel.sendMessage(
      'Успешно подключено к удаленному компьютеру',
      'SUCCESS'
    );
    this.users$ = this.webSocketService
      .connect$(
        `${this.configService.getVncWsUrl()}/ws/users/${this.id}?${this.tokenData.queryParameterName}=${this.tokenData.token}`
      )
      .pipe(
        catchError(e => {
          console.error(e);
